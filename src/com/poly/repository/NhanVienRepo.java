...
import com.poly.entity.NhanVienBackup;
...
public class NhanVienRepo {
    ...
    public List<NhanVien> getAllNhanVien() {
        sql = "SELECT id ,username, ten, email, so_dien_thoai, gioi_tinh, trang_thai, ngay_sinh, dia_chi, ma_dinh_danh, thoi_gian_tao, thoi_gian_sua,phan_quyen_id FROM nhan_vien";
    ...
    public int updateSinhVien(NhanVien nv, int id) {
        sql = "UPDATE nhan_vien SET username=?, ten=?, email=?, so_dien_thoai=?, gioi_tinh=?, trang_thai=?, ngay_sinh=?, dia_chi=?, ma_dinh_danh=?, thoi_gian_sua=? WHERE id=?";
        try {
            con = DBConnect.getConnection();
            ps = con.prepareStatement(sql);
    ...
    public List<NhanVien> findNhanVienByNameOrSoDienThoaiOrEmail(String keyword) {
        String sql = "SELECT id, username, ten, email, so_dien_thoai, gioi_tinh, trang_thai, ngay_sinh, dia_chi, ma_dinh_danh, thoi_gian_tao, thoi_gian_sua, phan_quyen_id FROM nhan_vien WHERE ten LIKE ? OR so_dien_thoai LIKE ? OR email LIKE ?";
        List<NhanVien> listNV = new ArrayList<>();
    ...
    public List<NhanVien> filterByGender(String gender) {
        List<NhanVien> listNV = new ArrayList<>();
        String sql = "SELECT id, username, ten, email, so_dien_thoai, gioi_tinh, trang_thai, "
                + "ngay_sinh, dia_chi, ma_dinh_danh, thoi_gian_tao, thoi_gian_sua, phan_quyen_id "
    ...
    public List<NhanVien> filterByStatusAndGender(String status, String gender) {
        List<NhanVien> listNV = new ArrayList<>();
        String sql = "SELECT id, username, ten, email, so_dien_thoai, gioi_tinh, trang_thai, "
                + "ngay_sinh, dia_chi, ma_dinh_danh, thoi_gian_tao, thoi_gian_sua, phan_quyen_id "
...
//        }
//    }
    public List<NhanVien> filterByStatus(String status) {
        String sql = "SELECT id, username, ten, email, so_dien_thoai, gioi_tinh, trang_thai, ngay_sinh, dia_chi, ma_dinh_danh, thoi_gian_tao, thoi_gian_sua, phan_quyen_id FROM nhan_vien WHERE trang_thai = ?";
        List<NhanVien> filteredList = new ArrayList<>();
        try (Connection con = DBConnect.getConnection(); PreparedStatement ps = con.prepareStatement(sql)) {
            ps.setString(1, status);
            ResultSet rs = ps.executeQuery();
...
//                                      Login
    ...
    // Hàm lấy danh sách Login từ CSDL
//    public List<Login> getAllLogins() {
//        String sql = "SELECT username, Password, role FROM phan_quyen";
...
//    }
    public List<Login> getAllLogins() {
        sql = "SELECT username, Password, Role FROM phan_quyen";
        List<Login> loginList = new ArrayList<>();
        try {
            con = DBConnect.getConnection();
            ps = con.prepareStatement(sql);
    ...
    public Login authenticateUser(String inputUsername, String inputPassword) {
        String sql = "SELECT role FROM phan_quyen WHERE username = ? AND Password = ?";
        ...
        try {
            ...
            if (rs.next()) {
                // If there is a match, return a User object
                String role = rs.getString("Role");
                return new Login(inputUsername, role);
            } else {
                // If there is no match, return null or handle it based on your requirement
                return null;
            }
        } catch (Exception e) {
            e.printStackTrace();
            // Handle exceptions and return null or handle it based on your requirement
            return null;
        }
    }

    // phan trang
    public List<NhanVien> getAllNhanVienPaged(int page, int rowsPerPage) {
        List<NhanVien> listNV = new ArrayList<>();
